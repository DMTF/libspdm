...
//
GLOBAL_REMOVE_IF_UNREFERENCED const uint8 m_ecd_test_root_cer[] = {
...
//
GLOBAL_REMOVE_IF_UNREFERENCED const uint8 m_ecd_test_pem_key[] = {
...
return_status validate_crypt_ecd_2(void)
{
	...
	my_print("\n- Ed-DSA Signing ... ");
	status = eddsa_sign(ecd_priv_key, CRYPTO_NID_NULL, message,
	...
	my_print("\n- Ed-DSA Verification ... ");
	status = eddsa_verify(ecd_pub_key, CRYPTO_NID_NULL, message,
			      sizeof(message), signature, sig_size);
	if (!status) {
		my_print("[Fail]");
		ecd_free(ecd_priv_key);
