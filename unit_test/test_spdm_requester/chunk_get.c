...
static uint16_t m_libspdm_local_req_msg_size_test_case_1;
    ...
    size_t sub_cert_index, size_t *sub_cert_count)
{
    ...
    *sub_cert_count = (m_libspdm_local_certificate_chain_size_test_case_1 +
                       m_libspdm_local_req_msg_size_test_case_1 - 1) /
                      m_libspdm_local_req_msg_size_test_case_1;

    if (sub_cert_index != *sub_cert_count - 1) {
        sub_cert_portion_length = m_libspdm_local_req_msg_size_test_case_1;
        ...
        sub_cert_portion_length = (uint16_t) (
            ...
            m_libspdm_local_req_msg_size_test_case_1 * (*sub_cert_count - 1));
    ...
    libspdm_copy_mem(
        ...
        m_libspdm_local_req_msg_size_test_case_1 * sub_cert_index,
    ...
    void* context, void* response, size_t* response_size)
{
    ...
    *response_size = sizeof(spdm_digest_response_t) +
    ...
    libspdm_zero_mem(digest, libspdm_get_hash_size(m_libspdm_use_hash_algo) * SPDM_MAX_SLOT_COUNT);
    ...
    void *spdm_context, size_t request_size, const void *request, uint64_t timeout)
{
    ...
    if (spdm_test_context->case_id == 0x1) {
        const spdm_get_certificate_request_t *spdm_request =
            (const void *)((const uint8_t *)request + header_size);
        if (spdm_request->header.request_response_code == SPDM_GET_CERTIFICATE) {
            m_libspdm_local_req_msg_size_test_case_1 = spdm_request->length;
        }
    ...
    void *spdm_context, size_t *response_size, void **response, uint64_t timeout)
{
    ...
    if (spdm_test_context->case_id == 0x1) {
        ...
        build_response_func = libspdm_requester_chunk_get_test_case2_build_measurements_response;
    } else if (spdm_test_context->case_id == 0x3) {
        build_response_func = libspdm_requester_chunk_get_test_case3_build_challenge_response;
    } else if (spdm_test_context->case_id == 0x4) {
    ...
    if (build_response_func) {
        ...
        if (sub_rsp_copied == 0) {
            ...
            sub_rsp = (spdm_message_header_t*) m_libspdm_local_large_response_buffer;
            sub_rsp_size = sizeof(m_libspdm_local_large_response_buffer);
            if (spdm_test_context->case_id == 0x5 || spdm_test_context->case_id == 0x6) {
                sub_rsp =
            ...
            *((uint32_t*) (chunk_rsp + 1)) = (uint32_t) sub_rsp_size;
            ...
            /* case_id 0x5 will only get 1 chunk message */
            if (spdm_test_context->case_id == 0x5) {
                sub_rsp_size = sub_rsp_copied;
            }
...
static void req_chunk_get_case1(void** state)
{
    ...
    #if LIBSPDM_RECORD_TRANSCRIPT_DATA_SUPPORT
    count = (data_size + m_libspdm_local_req_msg_size_test_case_1 - 1) / m_libspdm_local_req_msg_size_test_case_1;
