...
/*
 * Test 1: transport send fails before GET_CAPABILITIES leaves the Requester.
...
/**
 * Test 3: GET_CAPABILITIES is attempted before VERSION negotiation completes.
 * Expected behavior: returns with status LIBSPDM_STATUS_INVALID_STATE_LOCAL.
 **/
...
/**
 * Test 4: the Responder returns Busy and the Requester has no retries remaining.
...
/**
 * Test 5: the Responder returns RequestResynch.
 * Expected behavior: returns with status LIBSPDM_STATUS_RESYNCH_PEER and resets the
 * connection state.
 **/
...
/**
 * Test 7: the Responder returns ResponseNotReady to GET_CAPABILITIES.
 * Expected behavior: returns with status LIBSPDM_STATUS_NOT_READY_PEER.
 **/
 ...
 **/
static void req_get_capabilities_case31_supported_algorithms(void **state)
{
    libspdm_return_t status;
    libspdm_test_context_t *spdm_test_context;
    libspdm_context_t *spdm_context;
    ...
    assert_int_equal(status, LIBSPDM_STATUS_SUCCESS);
    assert_int_equal(supported_algs_length,
                     sizeof(spdm_supported_algorithms_block_t) +
                     6 * sizeof(spdm_negotiate_algorithms_common_struct_table_t));
    supported_algorithms = (void *)supported_algs_buffer;
    assert_int_equal(supported_algorithms->param1, 6);
    assert_int_equal(supported_algorithms->length, supported_algs_length);
    assert_int_equal(supported_algorithms->measurement_specification,
                     SPDM_MEASUREMENT_SPECIFICATION_DMTF);
    assert_int_equal(supported_algorithms->base_asym_algo, m_libspdm_use_asym_algo);
    assert_int_equal(supported_algorithms->base_hash_algo, m_libspdm_use_hash_algo);
    struct_table = (void *)(supported_algorithms + 1);
    assert_int_equal(struct_table[0].alg_type,
                     SPDM_NEGOTIATE_ALGORITHMS_STRUCT_TABLE_ALG_TYPE_DHE);
    assert_int_equal(struct_table[1].alg_type,
                     SPDM_NEGOTIATE_ALGORITHMS_STRUCT_TABLE_ALG_TYPE_AEAD);
    assert_int_equal(struct_table[4].alg_type,
                     SPDM_NEGOTIATE_ALGORITHMS_STRUCT_TABLE_ALG_TYPE_REQ_PQC_ASYM_ALG);
    assert_int_equal(struct_table[5].alg_type,
                     SPDM_NEGOTIATE_ALGORITHMS_STRUCT_TABLE_ALG_TYPE_KEM_ALG);
    spdm_context->local_context.capability.ext_flags = 0;
}
...
/**
 * Test 46: the fixed SPDM 1.2 CAPABILITIES structure is present, but the payload
 * violates field validation.
 * Expected behavior: returns with status LIBSPDM_STATUS_INVALID_MSG_FIELD.
 **/
...
int libspdm_req_get_capabilities_test(void)
{
    const struct CMUnitTest test_cases[] = {
        ...
        cmocka_unit_test(req_get_capabilities_case31_supported_algorithms),
