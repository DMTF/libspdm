...
/**
 * Test 34: The secured message version is 1.2 and the Integrator has set the sequence number
 *          endianness to big-endian. After key exchange the session's endianness will be
...
static void req_key_exchange_case34(void **state)
{
    ...
    /* Set 277 sequence number endianness to big-endian. */
    spdm_context->sequence_number_endian = LIBSPDM_DATA_SESSION_SEQ_NUM_ENC_BIG_DEC_BIG;
    ...
    assert_int_equal(
        libspdm_secured_message_get_session_state(
            spdm_context->session_info[0].secured_message_context),
        LIBSPDM_SESSION_STATE_HANDSHAKING);
    assert_int_equal(((libspdm_secured_message_context_t *)spdm_context->session_info->
