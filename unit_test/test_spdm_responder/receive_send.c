...
/**
 * Test 17: libspdm_process_request() returns INVALID_PARAMETER when request is NULL
 * or request_size is 0.
 **/
static void libspdm_test_responder_receive_send_rsp_case17(void** state)
{
    libspdm_return_t status;
    libspdm_test_context_t *spdm_test_context;
    libspdm_context_t *spdm_context;
...
/**
 * Test 18: libspdm_process_request() successfully decodes a normal (non-session)
 * GET_VERSION message via the test transport layer.
 **/
static void libspdm_test_responder_receive_send_rsp_case18(void** state)
{
    libspdm_return_t status;
    libspdm_test_context_t *spdm_test_context;
    libspdm_context_t *spdm_context;
...
/**
 * Test 19: libspdm_register_get_response_func(), libspdm_register_session_state_callback_func(),
...
static void libspdm_test_responder_receive_send_rsp_case19(void** state)
{
    libspdm_test_context_t *spdm_test_context;
    libspdm_context_t *spdm_context;
    libspdm_session_info_t *session_info;
...
/**
 * Test 20: libspdm_get_response_func_via_request_code() returns NULL for a request code
 ...
 **/
static void libspdm_test_responder_receive_send_rsp_case20(void** state)
{
    libspdm_return_t status;
    libspdm_test_context_t *spdm_test_context;
    libspdm_context_t *spdm_context;
...
/**
 * Test 21: libspdm_build_response() when context->last_spdm_error.error_code is
...
/**
 * Test 22: libspdm_build_response() rejects a NULL *response, a zero *response_size,
 * and a zero context->last_spdm_request_size (INVALID_STATE_LOCAL).
 *
 * When session_id is NULL, my_response is derived from (and immediately zeroes) the
 * caller-supplied response/response_size before the *response==NULL / *response_size==0
 * checks run, so invalid values there would fault instead of hitting the checks. To reach
 * those checks safely, a valid session is set up so my_response is instead derived from the
 * internal scratch buffer, leaving *response and *response_size unread until the checks.
 **/
static void libspdm_test_responder_receive_send_rsp_case22(void** state)
{
    libspdm_return_t status;
    libspdm_test_context_t *spdm_test_context;
    libspdm_context_t *spdm_context;
    ...
    /* Set up a valid session so my_response is derived from the scratch buffer instead of
     * from *response, allowing the *response == NULL and *response_size == 0 checks to be
     * exercised without dereferencing an invalid response pointer first. */
    session_id = 0xFFFFFFFD;
    spdm_context->latest_session_id = session_id;
    session_info = &spdm_context->session_info[0];
    libspdm_session_info_init(spdm_context, session_info, session_id,
                              SECURED_SPDM_VERSION_11 << SPDM_VERSION_NUMBER_SHIFT_BIT, true);
    ...
    /* Zero context->last_spdm_request_size (session_id == NULL path). */
...
int libspdm_rsp_receive_send_test(void)
{
    const struct CMUnitTest test_cases[] = {
        ...
        cmocka_unit_test_setup(libspdm_test_responder_receive_send_rsp_case17,
                               libspdm_unit_test_reset_context),
        /* libspdm_process_request() successfully decodes a normal GET_VERSION message */
        cmocka_unit_test_setup(libspdm_test_responder_receive_send_rsp_case18,
                               libspdm_unit_test_reset_context),
        ...
        cmocka_unit_test_setup(libspdm_test_responder_receive_send_rsp_case19,
                               libspdm_unit_test_reset_context),
        /* libspdm_get_response_func_via_request_code() NULL-return branch */
        cmocka_unit_test_setup(libspdm_test_responder_receive_send_rsp_case20,
                               libspdm_unit_test_reset_context),
        /* libspdm_build_response() DECRYPT_ERROR / INVALID_SESSION error-code handling */
        cmocka_unit_test_setup(libspdm_test_responder_receive_send_rsp_case21,
                               libspdm_unit_test_reset_context),
