...
static void rsp_encap_key_update_case10(void **state)
{
    ...
    status = libspdm_get_encap_request_key_update(spdm_context, session_id,
                                                  SPDM_KEY_UPDATE_OPERATIONS_UPDATE_KEY,
...
/**
 * Test 12: an operation other than UpdateKey or VerifyNewKey is requested.
 * UpdateAllKeys is not legal in the encapsulated flow, as the Responder cannot
 * update the Requester's keys on its behalf.
 * Expected behavior: libspdm_get_encap_request_key_update returns
...
/**
 * Test 13: no session exists for the given session_id.
 * Expected behavior: libspdm_get_encap_request_key_update returns
 * LIBSPDM_STATUS_INVALID_STATE_LOCAL, as there is no encapsulated context to use.
 **/
static void rsp_encap_key_update_case13(void **state)
{
    ...
    encap_request_size = sizeof(encap_request);
    status = libspdm_get_encap_request_key_update(spdm_context, 0x12345678,
                                                  SPDM_KEY_UPDATE_OPERATIONS_UPDATE_KEY,
                                                  &encap_request_size, encap_request);
...
/**
 * Test 15: the UpdateKey operation is requested.
 * Expected behavior: libspdm_get_encap_request_key_update succeeds and
 * produces a request with the UpdateKey operation, which is retained as the
 * session's last encapsulated request.
 **/
...
/**
 * Test 16: the VerifyNewKey operation is requested, which also exercises
 * libspdm_create_update_session_data_key and
...
int libspdm_rsp_encap_key_update_test(void)
{
    const struct CMUnitTest test_cases[] = {
        ...
        /* get_encap_request_key_update: an illegal operation is requested */
