...
static void rsp_psk_finish_rsp_case13(void **state)
{
    ...
    m_libspdm_psk_finish_request1_size = sizeof(spdm_psk_finish_request_t) + hmac_size/2;
    response_size = sizeof(response);
    status = libspdm_get_response_psk_finish(spdm_context,
                                             m_libspdm_psk_finish_request1_size,
                                             &m_libspdm_psk_finish_request1,
...
/**
 * Test 16: SPDM version 1.4, with OpaqueData.
 * Expected behavior: the responder accepts the request and produces a valid
 * PSK_FINISH_RSP response message with integrator defined opaque data in the response.
 **/
static void rsp_psk_finish_rsp_case16(void **state)
{
    libspdm_return_t status;
    libspdm_test_context_t *spdm_test_context;
    libspdm_context_t *spdm_context;
    ...
    libspdm_secret_lib_psk_finish_opaque_data_size = 8;
    ...
    assert_int_equal(libspdm_read_uint16(ptr),
                     libspdm_secret_lib_psk_finish_opaque_data_size);
    assert_int_equal(response_size, sizeof(spdm_psk_finish_response_t) +
                     sizeof(uint16_t) + libspdm_read_uint16(ptr));
    spdm_response = (void *)response;
    assert_int_equal(spdm_response->header.request_response_code, SPDM_PSK_FINISH_RSP);
...
/**
 * Test 17: SPDM version 1.4 request declares opaque data larger than payload.
 * Expected behavior: the responder rejects with INVALID_REQUEST.
 **/
static void rsp_psk_finish_rsp_case17(void **state)
{
    libspdm_return_t status;
    libspdm_test_context_t *spdm_test_context;
    libspdm_context_t *spdm_context;
    ...
    response_size = sizeof(response);
    status = libspdm_get_response_psk_finish(
        spdm_context, sizeof(spdm_psk_finish_request_t) + sizeof(uint16_t) + 4,
        &m_libspdm_psk_finish_request3, &response_size, response);
    assert_int_equal(status, LIBSPDM_STATUS_SUCCESS);
    assert_int_equal(response_size, sizeof(spdm_error_response_t));
    spdm_response = (void *)response;
    assert_int_equal(spdm_response->header.request_response_code, SPDM_ERROR);
...
/**
 * Test 18: SPDM version 1.4 responder generates opaque response larger than protocol max.
 * Expected behavior: the responder returns UNSPECIFIED error.
 **/
static void rsp_psk_finish_rsp_case18(void **state)
{
    libspdm_return_t status;
    libspdm_test_context_t *spdm_test_context;
    libspdm_context_t *spdm_context;
...
int libspdm_rsp_psk_finish_rsp_test(void)
{
    const struct CMUnitTest test_cases[] = {
        ...
        cmocka_unit_test(rsp_psk_finish_rsp_case16),
        /* SPDM 1.4 request opaque size mismatch */
