...
/**
 * Test 31: SPDM version 1.4, with OpaqueData.
 * Expected behavior: the responder accepts the request and produces a valid
...
static void rsp_finish_rsp_case31(void **state)
{
    ...
    g_generate_finish_opaque_data = true;
    libspdm_secret_lib_finish_opaque_data_size = 8;
    ...
    /* The ResponderVerifyData field shall be absent.*/
    ptr = (uint8_t *)response + sizeof(spdm_finish_response_t);
    assert_int_equal(libspdm_read_uint16(ptr),
                     libspdm_secret_lib_finish_opaque_data_size);
    assert_int_equal(response_size,
                     sizeof(spdm_finish_response_t) + sizeof(uint16_t) +
                     libspdm_read_uint16(ptr));
    spdm_response = (void *)response;
    assert_int_equal(spdm_response->header.request_response_code,
                     SPDM_FINISH_RSP);
    g_generate_finish_opaque_data = false;
    free(data1);
}
...
/**
 * Test 34: receiving a FINISH message with an incorrect MAC (a failed attempt), followed
 * by a retried FINISH message in the same session with a correct MAC.
 * Expected behavior: the first attempt is rejected with a DecryptError, and the message_f
 * transcript accumulated by that failed attempt is discarded so the retried FINISH with a
 * correct MAC is accepted, instead of also failing due to a polluted transcript.
 **/
static void rsp_finish_rsp_case34(void **state)
{
    libspdm_return_t status;
    libspdm_test_context_t *spdm_test_context;
    libspdm_context_t *spdm_context;
    ...
    spdm_context->connection_info.version = SPDM_MESSAGE_VERSION_11 <<
                                            SPDM_VERSION_NUMBER_SHIFT_BIT;
    spdm_context->connection_info.connection_state = LIBSPDM_CONNECTION_STATE_NEGOTIATED;
    /* A prior test case in this group may leave last_spdm_request_session_id_valid set; this
     * case's transport is "handshake in the clear" (not in a session), so it must be false. */
    ...
    /* First (failed) attempt: an all-zero MAC that will not verify. This still causes the
     * responder to append the request bytes to message_f before the HMAC check fails. */
    ptr = m_libspdm_finish_request1.signature;
    libspdm_set_mem(ptr, hmac_size, (uint8_t)(0x00));
    m_libspdm_finish_request1_size = sizeof(spdm_finish_request_t) + hmac_size;
    response_size = sizeof(response);
    status = libspdm_get_response_finish(spdm_context,
                                         m_libspdm_finish_request1_size,
    ...
    /* Second (retried) attempt, in the same session, with a correctly computed MAC. If the
     * message_f transcript from the first attempt was not discarded, this HMAC computation
     * (which only accounts for a single FINISH request) will not match what the responder
     * verifies against, and the retry will be incorrectly rejected. */
    libspdm_init_managed_buffer(&th_curr, sizeof(th_curr.buffer));
    cert_buffer = (uint8_t *)data1;
    cert_buffer_size = data_size1;
    libspdm_hash_all(m_libspdm_use_hash_algo, cert_buffer, cert_buffer_size, cert_buffer_hash);
...
int libspdm_rsp_finish_rsp_test(void)
{
    const struct CMUnitTest test_cases[] = {
        ...
        cmocka_unit_test_setup(rsp_finish_rsp_case32, libspdm_unit_test_reset_context),
        ...
        #endif /* (LIBSPDM_ENABLE_CAPABILITY_ENCAP_CAP) && (..) */
        /* Retried FINISH after a failed MAC verification must reset the message_f transcript */
        cmocka_unit_test(rsp_finish_rsp_case34),
    };
