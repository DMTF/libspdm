 ...
 * Expected behavior: the Responder produces a FINISH_RSP and starts the session watchdog.
 **/
static void rsp_finish_rsp_case34(void **state)
{
    ...
    /* Both endpoints support Heartbeat and the Responder offers a non-zero HeartbeatPeriod, so
     * the Responder starts the session watchdog. */
    spdm_context->connection_info.capability.flags |=
...
/**
 * Test 35: receiving a correct FINISH from the Requester when both endpoints support
 * Heartbeat but the Responder's HeartbeatPeriod is zero.
 * Expected behavior: the Responder produces a FINISH_RSP and does not start a watchdog, as a
 * session on which a heartbeat is not desired does not time out.
 **/
static void rsp_finish_rsp_case35(void **state)
{
    libspdm_return_t status;
    libspdm_test_context_t *spdm_test_context;
    libspdm_context_t *spdm_context;
    ...
    /* Both endpoints support Heartbeat but the Responder offers a HeartbeatPeriod of zero,
     * which means that a heartbeat is not desired on this session. */
    spdm_context->connection_info.capability.flags |=
        SPDM_GET_CAPABILITIES_REQUEST_FLAGS_HBEAT_CAP;
    spdm_context->local_context.capability.flags |=
        SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_HBEAT_CAP;
