...
static void rsp_chunk_response_case15(void** state)
{
    ...
    size_t response_size;
    uint8_t response[LIBSPDM_MAX_SPDM_MSG_SIZE];
    spdm_chunk_response_response_14_t* spdm_response;
    spdm_chunk_get_request_14_t spdm_request;
    void* scratch_buffer;
    size_t scratch_buffer_size;
    ...
    /* Fill 1st chunk with 1, 2nd chunk with 2, 3rd chunk with 3 */
    first_chunk_size = data_transfer_size -
                       (sizeof(spdm_chunk_response_response_14_t) + sizeof(uint32_t));
    second_chunk_size = data_transfer_size - sizeof(spdm_chunk_response_response_14_t);
    third_chunk_size = data_transfer_size - sizeof(spdm_chunk_response_response_14_t);
    total_chunk_size = first_chunk_size + second_chunk_size + third_chunk_size;
    LIBSPDM_ASSERT(total_chunk_size <= scratch_buffer_size);
    ...
    spdm_response = (spdm_chunk_response_response_14_t*) response;
...
static void rsp_chunk_response_case18(void** state)
{
    ...
    total_chunk_size = first_chunk_size + second_chunk_size + third_chunk_size + fourth_chunk_size;
    expected_response_size = sizeof(spdm_chunk_response_response_14_t) + fourth_chunk_size;
    LIBSPDM_ASSERT(total_chunk_size <= scratch_buffer_size);
