...
uintn m_libspdm_psk_finish_request1_size = sizeof(m_libspdm_psk_finish_request1);

libspdm_psk_finish_request_mine_t m_libspdm_psk_finish_request2 = {
    { SPDM_MESSAGE_VERSION_11, SPDM_PSK_FINISH, 0, 0 },
};
uintn m_libspdm_psk_finish_request2_size = LIBSPDM_MAX_MESSAGE_BUFFER_SIZE;
...
void libspdm_test_responder_psk_finish_case12(void **state)
{
    ...
    libspdm_hmac_all(m_libspdm_use_hash_algo, libspdm_get_managed_buffer(&th_curr),
                     libspdm_get_managed_buffer_size(&th_curr), request_finished_key,
                     hash_size, ptr);
    copy_mem(ptr, sizeof(m_libspdm_psk_finish_request1.verify_data),
