 ...
 **/
static void rsp_set_key_pair_info_ack_case8(void **state)
{
    libspdm_return_t status;
    libspdm_test_context_t *spdm_test_context;
    libspdm_context_t *spdm_context;
...
static void rsp_set_key_pair_info_ack_case7(void **state)
{
    ...
    for (victim_key_pair_id = 1;
        ...
        victim_asym_algo = victim_key_pair_info->current_asym_algo;
        victim_pqc_asym_algo = victim_key_pair_info->current_pqc_asym_algo;
        victim_assoc_cert_slot_mask = victim_key_pair_info->assoc_cert_slot_mask;
        ...
        for (other_key_pair_id = 1; other_key_pair_id <= total_key_pairs; other_key_pair_id++) {
            ...
            if ((other_key_pair_info->current_asym_algo == victim_asym_algo) &&
                (other_key_pair_info->current_pqc_asym_algo == victim_pqc_asym_algo)) {
                uint8_t slot_index;
...
static void rsp_set_key_pair_info_ack_case6(void **state)
{
    ...
    *ptr = desired_assoc_cert_slot_mask;
    ...
    assert_int_equal(libspdm_get_key_usage_mask(spdm_context, slot_id),
    ...
    *ptr = desired_assoc_cert_slot_mask;
    ...
    assert_int_equal(libspdm_get_key_usage_mask(spdm_context, slot_id), 0);
...
#if LIBSPDM_ENABLE_CAPABILITY_CERT_CAP
        cmocka_unit_test(rsp_set_key_pair_info_ack_case8),
#endif
    };
