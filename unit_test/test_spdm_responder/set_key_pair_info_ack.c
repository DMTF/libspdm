...
static void rsp_set_key_pair_info_ack_case6(void **state)
{
    ...
    /* Step 1: ParameterChange associating slot 4 with key_pair_id 4 and KeyExUse. Do not change the
    * algorithm (DesiredAsymAlgo = 0). This is a no-reset flow (SPDM 1.3), so it applies at once. */
    desired_key_usage = SPDM_KEY_USAGE_BIT_MASK_KEY_EX_USE;
    desired_asym_algo = 0;
    desired_assoc_cert_slot_mask = 0x10;
    set_key_pair_info_request_size =
        sizeof(spdm_set_key_pair_info_request_t) +
        sizeof(uint8_t) + sizeof(uint16_t) + sizeof(uint32_t) + sizeof(uint8_t);
    ...
    *ptr = desired_assoc_cert_slot_mask;
    ...
    /* Step 2: ParameterChange removing the slot 4 association (empty mask). */
    desired_key_usage = 0;
    desired_asym_algo = 0;
