...
static void rsp_slot_management_case7(void **state)
{
    ...
    if (libspdm_read_slot_management_certificate_chain(
            spdm_context, bank_id, 1, &expected_cert_chain_size,
            expected_cert_chain)) {
...
static void rsp_slot_management_case8(void **state)
{
    ...
    *ptr = sizeof(uint32_t); /* select_pqc_asym_algo_len, value 0 */
    ...
    /* Restore the Bank's original algorithm. The sample's Bank table is module-global and not reset
     * between test cases (only the NVM files are), so leaving Bank 0 cleared would break later cases
     * (e.g. case 16's SetCertificate). The Bank is unprovisioned here, so re-selecting its original
     * algorithm succeeds. */
    manage_bank->select_asym_algo = bank_asym_algo;
    response_size = sizeof(response);
    status = libspdm_get_response_slot_management(
