...
The example is from https://datatracker.ietf.org/doc/html/draft-ietf-lamps-dilithium-certificates
...
1. Generate DER / PEM public certificate.

Use https://github.com/open-quantum-safe/oqs-provider.

NOTE: The OID definition in https://datatracker.ietf.org/doc/html/draft-ietf-lamps-dilithium-certificates is
...
mkdir $PQC_ALGO
cd $PQC_ALGO
../openssl req -nodes -x509 -days 3650 -newkey $PQC_ALGO -keyout ca.key -out ca.cert -subj "/CN=DMTF libspdm $PQC_ALGO ca" -config ../openssl_spdm.cnf -provider oqsprovider -provider default
../openssl req -nodes -newkey $PQC_ALGO -keyout inter.key -out inter.req -batch -subj "/CN=DMTF libspdm $PQC_ALGO intermediate cert" -config ../openssl_spdm.cnf -provider oqsprovider -provider default
../openssl req -nodes -newkey $PQC_ALGO -keyout end_requester.key -out end_requester.req -batch -subj "/CN=DMTF libspdm $PQC_ALGO requseter cert" -config ../openssl_spdm.cnf -provider oqsprovider -provider default
../openssl req -nodes -newkey $PQC_ALGO -keyout end_responder.key -out end_responder.req -batch -subj "/CN=DMTF libspdm $PQC_ALGO responder cert" -config ../openssl_spdm.cnf -provider oqsprovider -provider default
...
# second slot
../openssl req -nodes -x509 -days 3650 -newkey $PQC_ALGO -keyout ca1.key -out ca1.cert -subj "/CN=DMTF libspdm $PQC_ALGO ca1" -config ../openssl_spdm.cnf -provider oqsprovider -provider default
...
If the end_requester.key.der and end_responder.key.der are not genrated, user can use `gen_pqc_key` program to generate raw binary key for ML-DSA-44, ML-DSA-65, ML-DSA-87.
