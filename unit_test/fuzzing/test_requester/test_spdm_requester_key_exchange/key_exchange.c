                                          ...
                                          IN OUT void *response, IN uint64_t timeout)
{
    ...
    temp_buff_size = sizeof(spdm_key_exchange_response_t) + dhe_key_size + 0 + sizeof(uint16_t) +
    ...
    dhe_context = libspdm_dhe_new(
        spdm_response->header.spdm_version << SPDM_VERSION_NUMBER_SHIFT_BIT, m_use_dhe_algo, TRUE);
    ...
    *(uint16_t *)ptr = (uint16_t)opaque_key_exchange_rsp_size;
    ...
    libspdm_hmac_all(m_use_hash_algo, m_zero_filled_buffer, hash_size, final_key, final_key_size,
                     handshake_secret);
