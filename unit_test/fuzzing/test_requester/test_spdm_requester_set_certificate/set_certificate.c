...
void libspdm_run_test_harness(void *test_buffer, size_t test_buffer_size)
{
    ...
    m_libspdm_requester_set_certificate_test_context.test_buffer = test_buffer;
    m_libspdm_requester_set_certificate_test_context.test_buffer_size =
        test_buffer_size;

    /* Successful response to set certificate , Normal message*/
    libspdm_unit_test_group_setup(&State);
    libspdm_test_requester_set_certificate_case1(&State);
    libspdm_unit_test_group_teardown(&State);

    /* Successful response to set certificate , Secured message*/
    libspdm_unit_test_group_setup(&State);
    libspdm_test_requester_set_certificate_case2(&State);
    libspdm_unit_test_group_teardown(&State);
}
#else
size_t libspdm_get_max_buffer_size(void)
