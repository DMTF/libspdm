...
void test_spdm_responder_encap_challenge_case1(void **State)
{
    ...
    *(uint16_t *)ptr = 0;
    ...
    libspdm_responder_data_sign(spdm_version, SPDM_CHALLENGE_AUTH, m_use_asym_algo, m_use_hash_algo,
                                FALSE, m_local_buffer, m_local_buffer_size, ptr, &sig_size);
    ptr += sig_size;
...
void test_spdm_get_encap_request_challenge_case2(void **State)
{
    ...
    spdm_get_encap_request_challenge(spdm_context, &spdm_response_size, spdm_request);
    free(spdm_request);
}
