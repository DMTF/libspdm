...
3. For each populated certificate chain slot call `libspdm_get_certificate()` and check that each
   call is successful.
...
`GET_ENCAPSULATED_REQUEST` then, from the encapsulated flow handler function:

1. Call `libspdm_get_encap_request_get_digests()`.
