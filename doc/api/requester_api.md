...
If the Responder supports this extended capability (introduced in SPDM 1.3), the Responder will
include the Supported Algorithms Block in its CAPABILITIES response. This function stores the
Responder's supported algorithms in the provided `algorithms` parameter, allowing the Requester
to know the Responder's algorithm capabilities before sending a NEGOTIATE_ALGORITHMS request.
If the requester wishes to continue with the connection, they can call `libspdm_init_connection`
