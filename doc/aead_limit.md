...
* its own advertised maximum allowed sequence number, `(2 ^ local_AeadLimitExponent) - 1`, where
  `local_AeadLimitExponent = floor(log2(configured cap + 1))`. This is the rounded-down value put on
  the wire, not the raw `LIBSPDM_DATA_MAX_SPDM_SESSION_SEQUENCE_NUMBER`; for a configured cap that is
  not of the form `2 ^ e - 1` the two differ, and enforcing the raw cap would let this side send past
  the limit the peer derives from the advertised exponent.
...
So neither side's limit is ever raised by the other, and each side enforces exactly the limit it
advertised. The effective maximum sequence number is itself
...
from `min(local advertised limit, peer advertised limit)` and is owned by the negotiation result. `libspdm_set_data`
