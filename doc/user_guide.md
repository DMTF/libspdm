...
1. Initialize SPDM context
   ...
   1.2, register the device io functions, transport layer functions, and device buffer functions, then set up the scratch buffer.
   ...
   ```C
   parameter.location = LIBSPDM_DATA_LOCATION_LOCAL;
   libspdm_set_data (spdm_context, LIBSPDM_DATA_PEER_PUBLIC_ROOT_CERT, &parameter, peer_root_cert1, peer_root_cert_size1);
   libspdm_set_data (spdm_context, LIBSPDM_DATA_PEER_PUBLIC_ROOT_CERT, &parameter, peer_root_cert2, peer_root_cert_size2);
   libspdm_set_data (spdm_context, LIBSPDM_DATA_PEER_PUBLIC_ROOT_CERT, &parameter, peer_root_cert3, peer_root_cert_size3);
   ```
...
5. Manage an SPDM session
   ...
   libspdm_start_session (
       spdm_context,
       false, // use_psk; send KEY_EXCHANGE and FINISH
       NULL, 0,
   ...
   libspdm_start_session (
       spdm_context,
       true, // use_psk; send PSK_EXCHANGE and PSK_FINISH
...
0. Choose proper SPDM libraries.
   ...
   If the Responder supports measurement, implement [measlib](https://github.com/DMTF/libspdm/blob/main/include/hal/library/responder/measlib.h).

   If the Responder supports CSR signing, implement [csrlib](https://github.com/DMTF/libspdm/blob/main/include/hal/library/responder/csrlib.h) in a secure environment.
   ...
   0.6, if the device does not have access to a real-time clock then build with `-DX509_IGNORE_TIME=ON` so that the validity period of certificates is not checked. This applies to both the OpenSSL and Mbed TLS cryptography libraries.

   0.7, implement a proper spdm_device_secret_lib.
...
```
The log can be reused by calling `libspdm_reset_msg_log`. It retains the buffer given to
`libspdm_init_msg_log` but discards the buffer's contents and disables logging, so
