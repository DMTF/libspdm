...
These contradictions are exactly the symptoms of an **un-normalized** store: the same fact
 ...
 * They cannot disagree because they read the same rows.
 *
...
| `ManageBank` `ConfigAlgo` | no more than one algorithm bit is selected (all-zero is allowed and clears the Bank's algorithm) | `popcount(SelectAsymAlgo) + popcount(SelectPqcAsymAlgo) <= 1` | `InvalidRequest` (only when > 1 bit) |
...
> INSTALL RULE (`SET_CERTIFICATE` / `SetCertificate`). `SET_CERTIFICATE` sets — and only sets — a
> certificate onto an **existing** key: it never creates a key pair and never creates or re-points a
> slot's key association. The slot must be **already associated** (`SET_KEY_PAIR_INFO` first), and the
> request's `KeyPairID` must **equal** the existing association. The install enforces all three of:
...
>    pair's `public_key_info` (the chain-↔-key-pair match DSP0274 requires).
>
> Any of these matching checks failing causes the install to be rejected and any existing certificate
> to be **retained**: a failure of (1) or (2) is `InvalidRequest`; a failure of (3) is the Table 101
> certificate-chain verification failure (§"SET_CERTIFICATE", Table 101 notes: "the Responder should
> verify that contents of the certificate chain meet the requirements … for the requested certificate
> model **and key pair** … if it does not, the Responder shall retain the current certificate").
...
Cross-command consistency is guaranteed by declarative constraints rather than by hoping each command
writer keeps independent copies in sync.

### Key-pair ↔ slot association consistency

Without normalization, three independent arrays (`AssocCertSlotMask`, per-slot `KeyPairID` in
`GET_DIGESTS`, and the per-slot certificate's algorithm) are written separately and can drift.

In this model there is exactly **one** association fact — a row in `slot_key_assoc`. All three views
...
## Slot-state representability and transitions

Without a representation of "key present, no cert", `GenerateKeyPair` / `KeyPairErase` have no
slot-level effect and the state is unreachable.

Here, state 3 is simply a `slot` row + a `slot_key_assoc` row + **no** `cert_chain` row, which
...
* `SetCertificate` → `INSERT cert_chain` → state 3→4 (the slot must already be associated; the
  install does not create the assoc — see the install rule).
...
This section describes how the one normalized store behaves for writes and the `ResetRequired`
two-phase commit.
...
bool libspdm_db_config_bank_algo(libspdm_db_t *db,
                                 uint8_t bank_id, const libspdm_db_algo_t *algo);
                                                          /* requires CONFIG_ALGO; algo selects no
                                                           * more than one algorithm (all-zero
                                                           * CLEARS the bank's algo -> not-yet-
                                                           * selected); no slot in the bank may have
                                                           * an associated key pair (state 3 or 4);
                                                           * clears all slot settings on success */
 ...
 * the install rule (see the state-preconditions table), SET_CERTIFICATE does NOT create a key pair
 * or create/re-point the association; it requires all three of:
 ...
 *
 * Either way a pending install never outlives the erase that supersedes it: a pending shadow must
 * not outlive the committed state it belongs to. */
 ...
 * the key pair must allow association (CertAssocCap) and exist (public_key_info_len),
 ...
 * provisioned slot must have its key associated" invariant true through reset.
 *
...
bool libspdm_db_dissociate_slot_key(libspdm_db_t *db, uint8_t bank_id, uint8_t slot_id)
{
    ...
    /* a committed certificate must be erased before dissociating (state 4 -> 3 first). */
    if (db->cert_chain[bank_id][slot_id].present) {
        return false;
    }
    ...
    /* cascade-discard any staged reset-required cert queued under the association: with the
     * association gone it can no longer be committed onto an unassociated slot at reset. */
    libspdm_db_clear_pending_cert_chain(db, bank_id, slot_id);
    return true;
}
 ...
 *
 * ConfigAlgo selects NO MORE THAN ONE algorithm: more than one bit (across SelectAsymAlgo and
 * SelectPqcAsymAlgo) is rejected. An all-zero algo (no bit set in either field) is NOT rejected --
 * it CLEARS the bank's algorithm, returning it to the "not yet selected" state (DSP0274
 * CurrentAsymAlgo: "If no bits are set in both fields ... the asymmetric algorithm for this Bank has
 * not yet been selected"). Clearing is a configuration change like any other: it is subject to the
 * no-associated-slot precondition and clears all slot settings; clearing an already-unconfigured
 * bank is an idempotent success. */
bool libspdm_db_config_bank_algo(libspdm_db_t *db, uint8_t bank_id, const libspdm_db_algo_t *algo)
{
    ...
    /* No more than one algorithm bit total. All-zero is NOT rejected: it clears the bank's algorithm
     * (set below), but is still a configuration change subject to the unprovisioned-slots
     * precondition, so it is not short-circuited here. */
    has_asym = (algo->asym_algo != 0);
    has_pqc  = (algo->pqc_asym_algo != 0);
    if (has_asym && has_pqc) {
        return false;   /* both set: more than one algorithm */
    }
    ...
    /* A non-zero Select shall be in the bank's AvailableAsymAlgo / AvailablePqcAsymAlgo (DSP0274
     * Table 145). Available = Capabilities minus the algorithms already configured for another bank.
     ...
     * banks" half, so this is the "in Capabilities" half. An all-zero Select (clear) names no
     * algorithm and is exempt from both halves. */
    if (has_asym || has_pqc) {
        ...
        /* uniqueness across configured banks: the "minus algorithms assigned to another bank" half
         * of the AvailableAsymAlgo rule. */
     ...
     * and a dissociate already discarded each slot's pending cert, so no per-slot child state
     * remains; libspdm_db_clear_slot() asserts that invariant per slot. */
    for (slot_id = 0; slot_id < db->config.db_max_slots; slot_id++) {
 ...
 * so this never commits a chain onto an unassociated slot. */
...
The key change is to make the **association** (`slot_key_assoc`) and the
...
1. **No SPDM wire change.** This is a device-backend data model. No message format is altered.
