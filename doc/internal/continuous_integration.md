...
Since these tests are run daily, and the number of new commits would be small, a failure should
be fixed the next day.

For execution time, when all tests pass, the total test time should be less than two hours.
...
are collected and published to https://dmtf.github.io/libspdm/coverage_log/, multiple versions of
the same compiler are exercised, and cross-compilation is exercised. The tests are run against the
latest `HEAD` commit on the `main` branch.

Test failures at this tier point to more obscure bugs that slipped through daily testing. Also,
...
For execution time, when all tests pass, the total test time should be less than five hours.
