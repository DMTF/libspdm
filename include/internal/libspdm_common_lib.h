...
/**
 * Build the DSP0277 1.3 AEAD limit opaque element (AEADlimitOE) into an opaque data blob that was
 ...
 *
 * The AEADlimitOE element is only defined for secured message version 1.3 and later; for an older
 * secured_message_version this is a no-op (nothing is added and *data_out_size is set to the size
 * of the existing blob).
 *
 * @param  spdm_context            A pointer to the SPDM context.
 * @param  secured_message_version The secured message version this opaque data targets.
 * @param  data_out_size           On input, the size in bytes of the data_out buffer. On output,
 *                                 the total size in bytes of the opaque data blob.
 ...
 * allowed session sequence number to min(integrator-configured cap, (2 ^ effective_exponent) - 1),
 * where effective_exponent = min(local exponent, peer exponent) and exponent 64 maps to the
 * all-ones max. Only takes effect when the session's secured message version is >= 1.3.
 *
 * @param  spdm_context          A pointer to the SPDM context.
