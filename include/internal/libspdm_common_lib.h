...
typedef struct {
    ...
    /* Local Used slot id
     * (for responder, to identify local used slot
     *  in KEY_EXCHANGE bassed session establishment process) */
...
bool libspdm_verify_peer_digests(libspdm_context_t *spdm_context,
                                 const void *digest, uint8_t slot_mask);
