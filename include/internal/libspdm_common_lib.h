...
typedef struct {
    ...
    * Register can know the state after StartSession / EndSession. */
    ...
    /* Session context of the original request that triggered the ResponseNotReady flow. Per DSP0274,
     * the validity of the RESPOND_IF_READY request is defined by that original request, so the
     * RESPOND_IF_READY must arrive in the same session context (same in-session/out-of-session, and
     * if in a session the same session_id). */
