...
#define PCI_TDISP_SET_TDISP_CONFIG_RSP 0x0C
...
#define PCI_TDISP_SET_TDISP_CONFIG_REQ 0x8C
...
#define PCI_TDISP_FUNCTION_ID_REQUESTER_ID_MASK 0x0000FFFF
#define PCI_TDISP_FUNCTION_ID_REQUESTER_SEGMENT_MASK 0x00FF0000
#define PCI_TDISP_FUNCTION_ID_REQUESTER_SEGMENT_OFFSET 16
#define PCI_TDISP_FUNCTION_ID_REQUESTER_SEGMENT_VALID 0x01000000
...
#define PCI_TDISP_DSM_CAPS_XT_MODE_SUPPORTED 0x1
...
typedef struct {
    /* Byte address of the first 4K page. */
    uint64_t first_page;
...
#define PCI_TDISP_INTERFACE_INFO_XT_MODE_SUPPORTED 0x20
...
typedef struct {
    ...
    uint8_t stream_config;
} pci_tdisp_bind_p2p_stream_request_t;

#define PCI_TDISP_STREAM_CONFIG_XT_ENABLE_CONFIG 0x1
...
typedef struct {
     ...
     * uint8_t vendor_data[]; */
} pci_tdisp_vdm_request_t;

/* TDISP VDM_RESPONSE response */
typedef struct {
    pci_tdisp_header_t header;
    uint8_t registry_id;
    uint8_t vendor_id_len;
    /* uint8_t vendor_id[vendor_id_len];
     * uint8_t vendor_data[]; */
} pci_tdisp_vdm_response_t;

/* TDISP SET_TDISP_CONFIG_REQUEST request */
typedef struct {
    pci_tdisp_header_t header;
    uint32_t dsm_config;
} pci_tdisp_set_tdisp_config_request_t;

#define PCI_TDISP_DSM_CONFIG_XT_MODE_ENABLE 0x1
#define PCI_TDISP_DSM_CONFIG_XT_BIT_FOR_LOCKED_MSIX 0x2

/* TDISP SET_TDISP_CONFIG_RESPONSE response */
typedef struct {
    pci_tdisp_header_t header;
} pci_tdisp_set_tdisp_config_response_t;
...
#define PCI_TDISP_ERROR_CODE_VENDOR_SPECIFIC_ERROR 0xFF
