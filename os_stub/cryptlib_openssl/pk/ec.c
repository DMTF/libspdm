...
static bool get_evp_compatible_public_key(EVP_PKEY *evp_pkey,
                            ...
                            size_t public_key_size)
{
    ...
    owned = get_evp_compatible_public_key(evp_pkey, public_key, public_key_size,
                             ...
                             size_t *public_size)
{
    ...
    *public_size = len;
    ...
    if (evp_pkey_copy_downgraded(&evp_pkey, gen_pkey) <= 0) {
        goto cleanup_gen_pkey;
    }
    result = true;
                            ...
                            size_t *key_size)
{
    ...
    owned = get_evp_compatible_public_key(evp_pkey, peer_public, peer_public_size,
                                          &compatible_key, &compatible_key_size);
...
bool get_evp_compatible_public_key(EVP_PKEY *evp_pkey,
                                   const uint8_t *public_key, size_t public_key_size,
                                   uint8_t **compatible_key, size_t *comptable_key_size) {
    int half_size;
    bool owned = false;
    /* EVP compatible public key hack
     * EVP expect public key [0] = 0x04 for compressed or 0x00 for uncompressed */
    half_size = evp_pkey_get_half_size(evp_pkey);
    if (half_size * 2 == public_key_size) {
        public_key_size += 1;
        *compatible_key = OPENSSL_malloc(public_key_size);
        owned = true;

        (*compatible_key)[0] = 0x04;
        memcpy((*compatible_key)+1, public_key, public_key_size-1);
    } else {
        *compatible_key = (uint8_t *) public_key;
    }
