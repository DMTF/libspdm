...
{
    FILE *file = NULL;
    char file_pcrpath[2048];
    char file_iter[2];
    uint8_t *file_dest = NULL;
    int file_index = 0;
    size_t hash_size;
    ...
    switch (measurement_hash_algo) {
        ...
        case SPDM_ALGORITHMS_MEASUREMENT_HASH_ALGO_TPM_ALG_SM3_256:
            ...
            break;
        default:
            LIBSPDM_DEBUG((LIBSPDM_DEBUG_ERROR, "unsupported hash_alog %d", measurement_hash_algo));
            return 0;
    }
    ...
    snprintf(file_pcrpath, sizeof(file_pcrpath), "/sys/class/tpm/tpm0/pcr-sha%s/%d", hash_name, measurements_index);
    file = fopen(file_pcrpath, "r");
    if (file == NULL) {
        LIBSPDM_DEBUG((LIBSPDM_DEBUG_ERROR, "failed to read tpm pcr file for %d at %s: %s", measurements_index, file_pcrpath, strerror(errno)));
        return 0;
    }

    int r;
    file_dest = (uint8_t*)(measurement_block + 1);
    while ((r = fscanf(file, "%2s", file_iter)) != -1) {
        // Convert HEX string to HEX digits value
        file_dest[file_index++] = (file_iter[0] % 32 + 9) % 25 * 16 + (file_iter[1] % 32 + 9) % 25;
    }
    file_dest[file_index] = '\0';
    fclose(file);
