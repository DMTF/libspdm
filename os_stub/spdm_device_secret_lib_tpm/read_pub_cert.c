...
static bool get_certificate(const char *handle, uint32_t base_hash_algo, uint32_t base_asym_algo, void **data, size_t *size, void **hash, size_t *hash_size)
{
    bool result;
    void *cert_handler;
    void *cert;
    ...
    result = libspdm_tpm_get_certificate((void *)handle, &cert_handler);
    if (!result)
        return false;

    result = libspdm_tpm_dump_certificate(cert_handler, &cert, &cert_size);
    if (!result)
        return false;
...
static bool get_certificate_chain(const char *filename, uint32_t base_hash_algo, uint32_t base_asym_algo, void **data, size_t *size, void **hash, size_t *hash_size,
                                  bool is_requester_cert, bool is_device_cert_model)
{
    ...
    libspdm_read_input_file(filename, &cert, &cert_size);
    ...
    cert_chain->length = (uint32_t)cert_chain_size;
    result = libspdm_verify_cert_chain_data(cert, cert_size,
...
bool libspdm_read_requester_root_public_certificate(uint32_t base_hash_algo,
                                                    ...
                                                    size_t *hash_size)
{
    return get_certificate("handle:0x1500021", base_hash_algo, base_asym_algo, data, size, hash, hash_size);
}
...
{
    return get_certificate_chain("requester_certchain.der", base_hash_algo, req_base_asym_alg, data, size, hash, hash_size, false, true);
}
                                        ...
                                        void **data, size_t *size)
{
    ...
    result = libspdm_tpm_get_certificate((void *)"0x1500023", &cert_handler);
    if (!result)
        return false;
...
bool libspdm_read_responder_public_certificate_chain(
    ...
    size_t *size, void **hash, size_t *hash_size)
{
    return get_certificate_chain("responder_certchain.der", base_hash_algo, base_asym_algo, data, size, hash, hash_size, false, true);
}
...
bool libspdm_read_responder_public_certificate_chain_alias_cert_till_dev_cert_ca(
    ...
    size_t *size, void **hash, size_t *hash_size)
{
    libspdm_debug_print(LIBSPDM_DEBUG_ERROR, "libspdm_read_responder_public_certificate_chain_alias_cert_till_dev_cert_ca not yet implemented\n");
    return false;
}
...
bool libspdm_read_responder_public_certificate_chain_alias_cert(
    ...
    size_t *size, void **hash, size_t *hash_size)
{
    libspdm_debug_print(LIBSPDM_DEBUG_ERROR, "libspdm_read_responder_public_certificate_chain_alias_cert not yet implemented\n");
    return false;
}
