                                   ...
                                   bool is_device_cert_model)
{
    ...
    void *cert_handle, *cert;
    void *x509_ca_cert;
    size_t cert_size;

    if (!libspdm_tpm_get_private_key("handle:0x81000003", &context)) {
        return false;
    }

    if (!libspdm_tpm_get_certificate("handle:0x1500023", &cert_handle)) {
        return false;
    }

    if (!libspdm_tpm_dump_certificate(cert_handle, &cert, &cert_size)) {
        return false;
    }
