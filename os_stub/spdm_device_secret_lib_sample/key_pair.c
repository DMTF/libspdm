 ...
 * LIBSPDM_KEY_PAIR_STATIC_ASSERT checks there will fail to build until this value is updated.
 */
#define LIBSPDM_SAMPLE_MAX_PUBLIC_KEY_INFO_LEN 21

/* Compile-time assertion: fails the build with "message" if "condition" is false.
 * C11 compilers get the descriptive _Static_assert diagnostic. Older compilers - notably MSVC,
 * which builds libspdm in a pre-C11 mode - fall back to declaring an enumerator whose value is
 * divided by zero when "condition" is false, which is not a valid constant expression and so
 * fails the build as well.
 */
#if defined(__STDC_VERSION__) && (__STDC_VERSION__ >= 201112L)
#define LIBSPDM_KEY_PAIR_STATIC_ASSERT(condition, message) _Static_assert(condition, message)
#else
#define LIBSPDM_KEY_PAIR_STATIC_ASSERT_NAME2(line) libspdm_key_pair_static_assert_ ## line
#define LIBSPDM_KEY_PAIR_STATIC_ASSERT_NAME(line) LIBSPDM_KEY_PAIR_STATIC_ASSERT_NAME2(line)
#define LIBSPDM_KEY_PAIR_STATIC_ASSERT(condition, message) \
    enum { LIBSPDM_KEY_PAIR_STATIC_ASSERT_NAME(__LINE__) = 1 / (int)(!!(condition)) }
...
void libspdm_init_key_pair_info() {
    ...
    LIBSPDM_KEY_PAIR_STATIC_ASSERT(
