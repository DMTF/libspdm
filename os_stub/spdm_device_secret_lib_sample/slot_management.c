    ...
    uint8_t *bank_result)
{
    ...
    /* At most one asymmetric algorithm (traditional or PQC) may be selected ("no more than one").
     * An all-zero selection is permitted -- it clears the Bank's algorithm, returning it to the
     * "not yet selected" state (DSP0274 CurrentAsymAlgo: "If no bits are set ... the asymmetric
     * algorithm for this Bank has not yet been selected"). */
    if ((select_asym_algo != 0) && (select_pqc_asym_algo != 0)) {
        return false;
    }

    /* Selecting the Bank's existing algorithm (including all-zero when the Bank is already
     * unconfigured) is an idempotent no-op and always succeeds. */
     ...
     * to InvalidRequest. This applies only to a non-zero selection: clearing the algorithm
     * (all-zero) is not subject to uniqueness. (The selecting-its-own-algorithm case was handled
     * above.) */
    if ((select_asym_algo != 0) || (select_pqc_asym_algo != 0)) {
    ...
    /* The Bank is unprovisioned, so its algorithm may be reconfigured. For a non-zero selection,
     * accept only an algorithm the device can actually back (one that maps to a certificate the
     * sample store can provide). An all-zero selection clears the Bank's algorithm, returning it to
     * the "not yet selected" state. At most one of select_asym_algo / select_pqc_asym_algo is
     * non-zero here. */
    if (select_pqc_asym_algo != 0) {
        ...
        /* All-zero selection: clear the Bank's algorithm (return to "not yet selected"). */
        bank->asym_algo = 0;
        bank->pqc_asym_algo = 0;
    }
