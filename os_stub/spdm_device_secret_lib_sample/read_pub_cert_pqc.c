    ...
    size_t *size, void **hash, size_t *hash_size)
{
    ...
    *size = 0;
    ...
    res = libspdm_verify_pqc_cert_chain_data(file_data, file_size,
                                             pqc_asym_algo, base_hash_algo,
    ...
    size_t *size, void **hash, size_t *hash_size)
{
    ...
    *size = 0;
    ...
    res = libspdm_x509_pqc_set_cert_certificate_check(leaf_cert, leaf_cert_len,
                                                      pqc_asym_algo, base_hash_algo,
                                                      is_requester_cert, is_device_cert_model);
    if (!res) {
        free(file_data);
    ...
    size_t *size, void **hash, size_t *hash_size)
{
    ...
    *size = 0;
    ...
    res = libspdm_x509_pqc_certificate_check(leaf_cert, leaf_cert_len,
                                             pqc_asym_algo, base_hash_algo,
                                             is_requester_cert, is_device_cert_model);
    if (!res) {
        free(file_data);
    ...
    size_t *size, void **hash, size_t *hash_size)
{
    ...
    *size = 0;
    ...
    res = libspdm_verify_pqc_cert_chain_data(file_data, file_size,
                                             req_pqc_asym_alg, base_hash_algo,
                                             is_requester_cert, is_device_cert_model);
    if (!res) {
        free(file_data);
        free(cert_chain);
