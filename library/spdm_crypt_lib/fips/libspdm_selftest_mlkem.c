...
bool libspdm_fips_selftest_mlkem(void *fips_selftest_context)
{
    ...
    extern uint8_t excepted_cipher_text_mlkem_1024[1568];
    extern uint8_t excepted_shared_secret_mlkem_1024[32];
    ...
    /*KAT test*/
    if (cipher_text_size != sizeof(excepted_cipher_text_mlkem_1024)) {
    ...
    if (!libspdm_consttime_is_mem_equal(cipher_text, excepted_cipher_text_mlkem_1024,
                                        sizeof(excepted_cipher_text_mlkem_1024))) {
        LIBSPDM_DEBUG((LIBSPDM_DEBUG_INFO, "ML-KEM KAT failed \n"));
        libspdm_mlkem_free(kem_context);
        result = false;
        goto update;
    ...
    if (shared_secret_size != sizeof(excepted_shared_secret_mlkem_1024)) {
        LIBSPDM_DEBUG((LIBSPDM_DEBUG_INFO, "ML-KEM KAT failed \n"));
        libspdm_mlkem_free(kem_context);
        result = false;
        goto update;
    ...
    if (!libspdm_consttime_is_mem_equal(shared_secret, excepted_shared_secret_mlkem_1024,
                                        sizeof(excepted_shared_secret_mlkem_1024))) {
        LIBSPDM_DEBUG((LIBSPDM_DEBUG_INFO, "ML-KEM KAT failed \n"));
        libspdm_mlkem_free(kem_context);
        result = false;
        goto update;
