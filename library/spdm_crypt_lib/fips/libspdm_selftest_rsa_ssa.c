...
bool libspdm_fips_selftest_rsa_ssa(void *fips_selftest_context)
{
    ...
    rsa = libspdm_rsa_new();
    if (rsa == NULL) {
        LIBSPDM_DEBUG((LIBSPDM_DEBUG_INFO, "RSA_SSA rsa new failed \n"));
        result = false;
        goto update;
    }
    ...
    if (!result) {
        LIBSPDM_DEBUG((LIBSPDM_DEBUG_INFO, "RSA_SSA rsa set key failed \n"));
    ...
    result = libspdm_rsa_pkcs1_sign_with_nid(rsa, LIBSPDM_CRYPTO_NID_SHA256,
                                             ...
                                             signature, &sig_size);
    if (!result) {
        LIBSPDM_DEBUG((LIBSPDM_DEBUG_INFO, "RSA_SSA rsa sign failed \n"));
        libspdm_rsa_free(rsa);
        result = false;
        goto update;
    }
    ...
    if (sig_size != sizeof(expected_sig)) {
        LIBSPDM_DEBUG((LIBSPDM_DEBUG_INFO, "RSA_SSA KAT failed \n"));
        libspdm_rsa_free(rsa);
        result = false;
        goto update;
    }
    ...
    result = libspdm_rsa_pkcs1_verify_with_nid(rsa, LIBSPDM_CRYPTO_NID_SHA256,
                                               ...
                                               signature, sig_size);
    if (!result) {
        LIBSPDM_DEBUG((LIBSPDM_DEBUG_INFO, "RSA_SSA selftest failed \n"));
        libspdm_rsa_free(rsa);
        result = false;
        goto update;
    }
