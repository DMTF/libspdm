                              ...
                              libspdm_il1il2_managed_buffer_t *il1il2)
{
    ...
    if (is_mut) {
        ...
        if (spdm_session_info == NULL) {
            LIBSPDM_DEBUG((LIBSPDM_DEBUG_INFO, "message_mut_e data :\n"));
            LIBSPDM_INTERNAL_DUMP_HEX(
                libspdm_get_managed_buffer(&spdm_context->transcript.message_mut_e),
                libspdm_get_managed_buffer_size(&spdm_context->transcript.message_mut_e));
            status = libspdm_append_managed_buffer(
                il1il2,
                libspdm_get_managed_buffer(&spdm_context->transcript.message_mut_e),
                libspdm_get_managed_buffer_size(&spdm_context->transcript.message_mut_e));
        } else {
            LIBSPDM_DEBUG((LIBSPDM_DEBUG_INFO, "use message_mut_e in session :\n"));
            LIBSPDM_INTERNAL_DUMP_HEX(
                libspdm_get_managed_buffer(&spdm_session_info->session_transcript.message_mut_e),
                libspdm_get_managed_buffer_size(
                    &spdm_session_info->session_transcript.message_mut_e));
            status = libspdm_append_managed_buffer(
                il1il2,
                libspdm_get_managed_buffer(&spdm_session_info->session_transcript.message_mut_e),
                libspdm_get_managed_buffer_size(
