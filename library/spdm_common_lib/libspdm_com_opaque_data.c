                                         ...
                                         uint8_t peer_aead_limit_exponent)
{
    ...
    uint8_t local_aead_limit_exponent;
    uint64_t local_max_sequence_number;
     ...
     *
     * The DSP0277 AeadLimit is the first sequence number that is NOT allowed, so the maximum
     * allowed sequence number is AeadLimit - 1 = 2 ^ AeadLimitExponent - 1. Fold together this
     * endpoint's advertised limit and the peer's advertised limit; the effective cap is the
     * smaller of the two, so neither side's limit is ever raised by the other.
     *
     * This endpoint's contribution must be the SAME rounded-down limit it advertised on the wire,
     * not the raw configured max_spdm_session_sequence_number. When the configured cap is not of
     * the form 2 ^ e - 1 the advertised exponent is floor(log2(cap + 1)); enforcing the raw cap
     * here would let this side send past the limit the peer derives from that exponent. At
     * exponent 64 the maximum allowed value is 2^64 - 1, i.e. LIBSPDM_MAX_SPDM_SESSION_SEQUENCE_NUMBER
     * (all-ones), which is also the default. */
    local_aead_limit_exponent = libspdm_aead_limit_exponent_from_max_sequence_number(
        spdm_context->max_spdm_session_sequence_number);
    if (local_aead_limit_exponent >= 64) {
        local_max_sequence_number = LIBSPDM_MAX_SPDM_SESSION_SEQUENCE_NUMBER;
    } else {
        local_max_sequence_number = ((uint64_t)1 << local_aead_limit_exponent) - 1;
    }
    ...
    effective_max = local_max_sequence_number;
