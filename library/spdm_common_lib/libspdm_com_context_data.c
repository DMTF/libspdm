...
#if LIBSPDM_RECORD_TRANSCRIPT_DATA_SUPPORT
    if (spdm_session_info == NULL) {
        libspdm_reset_managed_buffer(&spdm_context->transcript.message_mut_e);
    } else {
        libspdm_reset_managed_buffer(&spdm_session_info->session_transcript.message_mut_e);
    }
#else
    if (spdm_session_info == NULL) {
        if (spdm_context->transcript.digest_context_mut_il1il2 != NULL) {
            libspdm_hash_free (spdm_context->connection_info.algorithm.base_hash_algo,
                               spdm_context->transcript.digest_context_mut_il1il2);
            spdm_context->transcript.digest_context_mut_il1il2 = NULL;
        }
    } else {
        if (spdm_session_info->session_transcript.digest_context_mut_il1il2 != NULL) {
            libspdm_hash_free (spdm_context->connection_info.algorithm.base_hash_algo,
                               spdm_session_info->session_transcript.digest_context_mut_il1il2);
            spdm_session_info->session_transcript.digest_context_mut_il1il2 = NULL;
        }
    }
#endif
}
                                                   ...
                                                   uint8_t request_code)
{
    ...
    case SPDM_GET_ENDPOINT_INFO:
        ...
        libspdm_reset_message_mut_e(spdm_context, session_info);
        break;
    default:
        break;
    }
...
#if LIBSPDM_RECORD_TRANSCRIPT_DATA_SUPPORT
    if (spdm_session_info == NULL) {
        return libspdm_append_managed_buffer(&spdm_context->transcript.message_mut_e,
                                             message, message_size);
    } else {
        return libspdm_append_managed_buffer(&spdm_session_info->session_transcript.message_mut_e,
                                             message, message_size);
    }
#else
    {
        ...
        if (spdm_session_info == NULL) {
            if (spdm_context->transcript.digest_context_mut_il1il2 == NULL) {
                spdm_context->transcript.digest_context_mut_il1il2 = libspdm_hash_new (
                    spdm_context->connection_info.algorithm.base_hash_algo);
                if (spdm_context->transcript.digest_context_mut_il1il2 == NULL) {
                    return LIBSPDM_STATUS_CRYPTO_ERROR;
                }
            ...
            if (spdm_session_info->session_transcript.digest_context_mut_il1il2 == NULL) {
                spdm_session_info->session_transcript.digest_context_mut_il1il2 = libspdm_hash_new (
                    spdm_context->connection_info.algorithm.base_hash_algo);
                if (spdm_session_info->session_transcript.digest_context_mut_il1il2 == NULL) {
                    return LIBSPDM_STATUS_CRYPTO_ERROR;
                }
                                                           ...
                                                           size_t num_secured_contexts)
{
    ...
    context->transcript.message_mut_e.max_buffer_size =
        sizeof(context->transcript.message_mut_e.buffer);
#endif
...
void libspdm_deinit_context(void *spdm_context)
{
    ...
    for (session_id = 0; session_id < LIBSPDM_MAX_SESSION_COUNT; session_id++) {
        ...
        libspdm_reset_message_mut_e(context, session_info);
