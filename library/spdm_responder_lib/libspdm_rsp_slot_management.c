    ...
    size_t *response_size, void *response)
{
    ...
    /* Defensive: the HAL writes num_slot_elements and slot_digest_size back as the actual values,
     * but slot_elements[] / slot_digests[] were sized for SPDM_MAX_SLOT_COUNT and a hash_size
     * stride. A HAL that reports more than the supplied capacity would otherwise overflow the
     * response buffer below; reject such an out-of-range result rather than trust it. */
    if ((num_slot_elements > SPDM_MAX_SLOT_COUNT) || (slot_digest_size > hash_size)) {
        return libspdm_generate_error_response(spdm_context,
    ...
    size_t *response_size, void *response)
{
    ...
    /* ConfigAlgo selects the Bank's algorithm: the total number of bits set in SelectAsymAlgo and
     * SelectPqcAsymAlgo shall be no more than one. (DSP0274 Table 145 says "exactly one", which
     * would reject an all-zero selection; this implementation takes the "no more than one" reading,
     ...
     * the responder validates here against the same HAL the GetBankDetails SubCode reports from. An
     * unknown BankID also fails the lookup and is reported as InvalidRequest. */
    ...
    size_t *response_size, void *response)
{
    ...
    bool result;
    size_t req_struct_offset;
    /* Out-params for the bank-existence check (libspdm_read_slot_management_bank_details); only the
     * call's success/failure is used here. */
    spdm_slot_management_slot_element_struct_t slot_elements[SPDM_MAX_SLOT_COUNT];
    uint8_t slot_digests[SPDM_MAX_SLOT_COUNT * LIBSPDM_MAX_HASH_SIZE];
    uint32_t slot_digest_size;
    uint8_t bank_attributes;
    ...
    old_local_cert_chain = spdm_context->local_context.local_cert_chain_provision[slot_id];
