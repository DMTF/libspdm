    ...
    void *spdm_context, libspdm_endpoint_info_device_func func)
{
    ...
    context->endpoint_info_device_func = func;
                                                    ...
                                                    void *response)
{
    ...
    if (spdm_context->endpoint_info_device_func == NULL) {
        return libspdm_generate_error_response(
            spdm_context, SPDM_ERROR_CODE_UNSUPPORTED_REQUEST,
            SPDM_GET_ENDPOINT_INFO, response_size, response);
    }
    ...
    status = spdm_context->endpoint_info_device_func(
        spdm_context, sub_code, &endpoint_info_size, NULL);
    ...
    *response_size = spdm_response_size;
    ...
    status = spdm_context->endpoint_info_device_func(
        spdm_context, sub_code, &endpoint_info_size, ptr);
    if (LIBSPDM_STATUS_IS_ERROR(status)) {
        return libspdm_generate_error_response(
            spdm_context, SPDM_ERROR_CODE_UNSUPPORTED_REQUEST,
            SPDM_GET_ENDPOINT_INFO, response_size, response);
