                                                       ...
                                                       void *response)
{
    ...
    /* Per DSP0274 RESPOND_IF_READY request message format: the validity of the RESPOND_IF_READY
    * request "is defined by the original request that caused the RESPOND_IF_READY flow. This means
    * the last request to which the Responder sent an ERROR message of ErrorCode=ResponseNotReady".
    * The RESPOND_IF_READY shall therefore arrive in the same session context as that original
    * request: the same in-session / out-of-session state, and, when in a session, the same
    * session_id. Otherwise the cached response would be returned outside the context it belongs
    * to. (Only one deferred request is tracked per connection; per DSP0274 a conformant Requester
    * does not have multiple outstanding requests to the same Responder within a connection.) */
