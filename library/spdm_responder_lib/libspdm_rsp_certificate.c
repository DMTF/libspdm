                                                  ...
                                                  void *response)
{
    ...
    /* A SlotSizeRequested query returns only a size and is independent of whether the slot
    * currently holds a certificate chain, so it must not be rejected for an empty slot. */
    slot_size_requested =
