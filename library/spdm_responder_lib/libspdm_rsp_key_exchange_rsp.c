                                                   ...
                                                   void *response)
{
    ...
    if (libspdm_is_capabilities_flag_supported(
            spdm_context, false,
            0, SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_CERT_CAP)) {
        if (slot_id >= SPDM_MAX_SLOT_COUNT) {
            return libspdm_generate_error_response(spdm_context,
                                                   SPDM_ERROR_CODE_INVALID_REQUEST, 0,
                                                   response_size, response);
        }
        if (spdm_context->local_context.local_cert_chain_provision[spdm_context->connection_info.current_bank][slot_id]
            ==
            NULL) {
            return libspdm_generate_error_response(spdm_context,
                                                   SPDM_ERROR_CODE_INVALID_REQUEST, 0,
    ...
    hmac_size = libspdm_get_hash_size(
        spdm_context->connection_info.algorithm.base_hash_algo);
    if (spdm_context->connection_info.algorithm.kem_alg != 0) {
        kem_encap_key_size = libspdm_get_kem_encap_key_size(
    ...
    if (opaque_data_length != 0) {
        ...
        if (use_default_opaque_data) {
            ...
            /* DSP0277 1.3: reserve room for this Responder's AEADlimitOE. The size helper returns 0
             * unless the negotiated secured message version is 1.3 or later. */
            opaque_key_exchange_rsp_size +=
        ...
        /* DSP0277 1.3: read the Requester's AEAD limit from the request (absent -> default 64).
         * This is independent of whether the Responder builds default or custom response opaque
         * data, so it runs for both paths. */
        status = libspdm_process_opaque_data_aead_limit(
            spdm_context, secured_message_version, opaque_data_length, req_opaque_data,
    ...
    *response_size = total_size;
    ...
    if (spdm_context->connection_info.algorithm.kem_alg != 0) {
        kem_context = libspdm_secured_message_kem_new(
            spdm_context->connection_info.version,
            spdm_context->connection_info.algorithm.kem_alg, false);
        if (kem_context == NULL) {
            libspdm_free_session_id(spdm_context, session_id);
            return libspdm_generate_error_response(spdm_context,
                                                   SPDM_ERROR_CODE_UNSPECIFIED, 0,
        ...
        result = libspdm_secured_message_kem_encapsulate(
            spdm_context->connection_info.algorithm.kem_alg,
            kem_context,
            (const uint8_t *)request + sizeof(spdm_key_exchange_request_t),
            kem_encap_key_size, ptr, &kem_cipher_text_size, session_info->secured_message_context);
        ...
        LIBSPDM_DEBUG((LIBSPDM_DEBUG_INFO, "Calc Self cipher_text (0x%zx):\n", kem_cipher_text_size));
        LIBSPDM_INTERNAL_DUMP_HEX(ptr, kem_cipher_text_size);

        LIBSPDM_DEBUG((LIBSPDM_DEBUG_INFO, "Calc peer encap_key (0x%zx):\n", kem_encap_key_size));
        LIBSPDM_INTERNAL_DUMP_HEX((const uint8_t *)request +
                                  sizeof(spdm_key_exchange_request_t),
                                  kem_encap_key_size);

        ptr += kem_cipher_text_size;
    } else {
    ...
    if (opaque_key_exchange_rsp_size != 0) {
        if (use_default_opaque_data) {
            size_t version_selection_size =
                libspdm_get_opaque_data_version_selection_data_size(spdm_context);
            libspdm_build_opaque_data_version_selection_data(
                spdm_context, secured_message_version, &version_selection_size, ptr);
            /* DSP0277 1.3: advertise this Responder's own AEAD limit. opaque_key_exchange_rsp_size
             * is the reserved opaque data capacity (version selection + AEAD limit); the append is a
             * no-op unless the negotiated secured message version is 1.3 or later. */
