                                                         ...
                                                         void *response)
{
    ...
    case LIBSPDM_RESPONSE_STATE_NOT_READY:
         ...
         *
         * Only one deferred (ResponseNotReady) request is tracked per connection. Per DSP0274
         * "Requirements for Requesters": "A Requester shall not have multiple outstanding requests
         * to the same Responder within a connection" (the only exceptions are GET_VERSION and the
         * large-message / CHUNK_SEND pair). A request awaiting its RESPOND_IF_READY completion is
         * still outstanding, so a conformant Requester will not issue another request - in another
         * session or otherwise - until this one completes. Per DSP0274 "Requirements for
         * Responders", a Responder "is not required to process more than one request message at a
         * time, even across connections", so a single context-wide cache is sufficient and
         * conformant. */
        if (request_code != SPDM_RESPOND_IF_READY) {
            ...
            /* Record the session context of the original request. Per DSP0274 RESPOND_IF_READY
             * request message format: the validity of the RESPOND_IF_READY request "is defined by
             * the original request that caused the RESPOND_IF_READY flow". The matching
             * RESPOND_IF_READY must therefore arrive in the same session context. */
