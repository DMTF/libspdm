                                                  ...
                                                  void *response)
{
    ...
    LIBSPDM_ASSERT(spdm_request->header.request_response_code == SPDM_GET_MEASUREMENT_EXTENSION_LOG);
    ...
    *response_size = sizeof(spdm_measurement_extension_log_response_t) + length;
    ...
    libspdm_copy_mem(spdm_response + 1, response_capacity - sizeof(spdm_measurement_extension_log_response_t),
                     (const uint8_t *)spdm_context->local_context
                     .local_cert_chain_provision[slot_id] + offset, length);

    if (session_info == NULL) {
        /* Log to transcript. */
        const size_t spdm_request_size = sizeof(spdm_get_measurement_extension_log_request_t);
