                                                ...
                                                void **message)
{
    ...
    security_protocol_specific = BSWAP16(storage_header->security_protocol_specific);
#else
...
#if CMAKE_C_BYTE_ORDER == LITTLE_ENDIAN
    storage_length = BSWAP32(storage_header->length);
#else
    ...
    *transport_message = (uint8_t *)message - sizeof(storage_spdm_transport_header);
    ...
    if (session_id != NULL) {
        storage_header->security_protocol_specific = SPDM_STORAGE_OPERATION_CODE_SECURED_MESSAGE <<
                                                     2;
        storage_header->security_protocol_specific |= connection_id & 0x3;
...
#if CMAKE_C_BYTE_ORDER == LITTLE_ENDIAN
    storage_header->length = BSWAP32(storage_header->length);
    storage_header->security_protocol_specific = BSWAP16(
    ...
    void *transport_message)
{
    ...
    *transport_message_size = sizeof(spdm_storage_discovery_response_t);
    ...
    /* max_connection_id = 0 => 1 supported connection */
    discovery_response->max_connection_id = 0;
    discovery_response->supported_operations[0] = (1 << SPDM_STORAGE_OPERATION_CODE_DISCOVERY)
