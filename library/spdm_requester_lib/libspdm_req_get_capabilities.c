                                                     ...
                                                     void *supported_algs)
{
    ...
    if (spdm_response->header.spdm_version >= SPDM_MESSAGE_VERSION_13 &&
        ...
        spdm_supported_algorithms_block_t *supported_algorithms =
            (spdm_supported_algorithms_block_t*)((uint8_t*)spdm_response + spdm_response_size);

        if (supported_algorithms->ext_asym_count > 0) {
            spdm_response_size += supported_algorithms->ext_asym_count *
                                  sizeof(spdm_extended_algorithm_t);
        }
        if (supported_algorithms->ext_hash_count > 0) {
            spdm_response_size += supported_algorithms->ext_hash_count *
                                  sizeof(spdm_extended_algorithm_t);
        }
        if (supported_algorithms->param1 > 0) {
            spdm_response_size += supported_algorithms->param1 *
                                  sizeof(spdm_negotiate_algorithms_common_struct_table_t);
        }

        spdm_response_size += sizeof(spdm_supported_algorithms_block_t);
...
libspdm_return_t libspdm_get_capabilities(libspdm_context_t *spdm_context)
{
    ...
    do {
        status = libspdm_try_get_capabilities(spdm_context, false, NULL, NULL);
                                                              ...
                                                              void *supported_algs)
{
    ...
    do {
        status =
            libspdm_try_get_capabilities(spdm_context, get_supported_algorithms, supported_algs_length, supported_algs);
        if (status != LIBSPDM_STATUS_BUSY_PEER) {
            return status;
        }
