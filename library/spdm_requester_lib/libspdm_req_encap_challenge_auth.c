    ...
    size_t *response_size, void *response)
{
    ...
    total_size =
        ...
        sizeof(uint16_t) +
        context->local_context.opaque_challenge_auth_rsp_size +
        signature_size;
    ...
    *response_size = total_size;
    ...
    libspdm_write_uint16 (ptr, (uint16_t)context->local_context
                          .opaque_challenge_auth_rsp_size);
    ptr += sizeof(uint16_t);
    if (context->local_context.opaque_challenge_auth_rsp != NULL) {
        libspdm_copy_mem(ptr,
                         response_capacity - (ptr - (uint8_t*)response),
                         context->local_context.opaque_challenge_auth_rsp,
                         context->local_context.opaque_challenge_auth_rsp_size);
        ptr += context->local_context.opaque_challenge_auth_rsp_size;
    }
